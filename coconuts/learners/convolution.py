...
class BaseCNN(BaseNNLearner):
    ...
    def _compute_kernel_dim(input_size, kernel_size, padding, stride=1, dilation=1):
        if kernel_size > input_size:
            raise ValueError(
                "Kernel size cannot be larger than input size. %d > %d" % (kernel_size, input_size)
            )
        return math.floor(
    ...
    def input_to_tensor(self, arr: numpy.ndarray, flatten: bool = False):
        if flatten:
            raise ValueError("Convolution type requires 2D so `flatten` cannot be true")
        ...
        if len(input_dim) == 1:
            ...
            if math.floor(sqrt) != math.ceil(sqrt):
                ...
                if getattr(self, "verbose", False) and not hasattr(self, "model_"):
                    warnings.warn(
                        "Input is 1D and will be padded to be converted to 2D because %d "
                        "is not a squared number." % input_dim[0],
                        RuntimeWarning,
                    )
        ...
        if len(input_dim) != 3:
            raise ValueError(
                "Input expected to be 3D, instead found %d %r" % (len(input_dim), input_dim)
            )
    ...
    def init_model(self, input_shape: tuple, output_shape: tuple):
        ...
        conv_dim_out = [
            BaseCNN._compute_kernel_dim(dim, self.kernel_size, self.padding)
            for dim in input_shape[1:]
        ]
        # http://pytorch.org/docs/master/nn.html#torch.nn.MaxPool2d
        maxpool_dim_out = [
            BaseCNN._compute_kernel_dim(dim, self.maxpool_size, 0, stride=self.maxpool_size)
            for dim in conv_dim_out
        ]
        ...
        self.print(
            "Sequential layers:", self.kernel_size, conv_dim_out, maxpool_dim_out, flat_dim_out
        )
