...
class TransferLearningModel(BaseNNLearner):
    ...
    def __init__(self, base_model: (BaseNNLearner, nn.Module), discard_layer_count: int = 1,
                 freeze_base_model: bool = True, learning_rate: float = .001,
                 loss_function: str = None, random_seed: int = 0, verbose: bool = False, **kwargs):
        super().__init__(base_model=base_model, discard_layer_count=discard_layer_count,
                         freeze_base_model=freeze_base_model, learning_rate=learning_rate,
                         loss_function=loss_function, random_seed=random_seed, verbose=verbose,
                         **kwargs)
        ...
        if isinstance(base_model, nn.Module):
            ...
            for mixin in (BaseNNClassifier, BaseNNRegressor, HighDimensionalMixin):
                if isinstance(base_model, mixin): self.apply_mixin(mixin)
    ...
    def init_model(self, input_shape: Tuple[int], output_shape: Tuple[int]):
        ...
        custom_model: nn.Module = nn.Sequential(
            *flat_module_list(self.base_model)[:-self.discard_layer_count], self._flattener).eval()
        ...
        return nn.Sequential(
            *trained_layers, nn.Linear(sample_output.shape[-1], output_shape[0]))
